...
class MTDataEntity:
    ...
    def __init__(self, dataItem, value):
        ...
        if dataItem.category == "SAMPLE" and (
            not isinstance(value, numbers.Number) and value != "UNAVAILABLE"
        ):
        ...
        if dataItem.category == "CONDITION" and value not in [
            "UNAVAILABLE",
            "NORMAL",
            "WARNING",
            "FAULT",
            ...
            raise ValueError(
                "CONDITION value must be one of UNAVAILABLE, NORMAL, WARNING, or FAULT"
            )
