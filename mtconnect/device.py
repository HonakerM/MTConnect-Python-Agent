...
def tree_helper(component, level=0):
    ...
    for item in component.get_sub_items():
        output_string = (
            output_string + "+" + ("-" * 5) * (level + 1) + "+" + str(item.id) + "\n"
        )
...
class MTGeneric:
    ...
    def __str__(self):
        return "{} with id {} and name {} with parent {}".format(
            type(self), self.id, self.name, self.parent_component
        )
...
class MTComponent(MTGenericContainer):
    ...
    def __init__(
        self, id, name, type, xml_data, parent_component, device, description=None
    ):
...
class MTDataItem(MTGeneric):
    ...
    def get_data(self, start_sequence=float("-inf"), end_sequence=float("inf")):
        ...
        for data in self.data_list:
            if (
                data.sequence_number > start_sequence
                and data.sequence_number <= end_sequence
            ):
